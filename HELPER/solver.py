...
class Solver(object):
    ...
    def _step(self):
       ...
       #决定pdg训练的样本
        adv_mask = torch.rand(self.batch_size) < self.adv_ratio  # self.adv_ratio是对抗样本的比例
        X_batch_adv = X_batch[adv_mask]
        y_batch_adv = y_batch[adv_mask]
        #pgd攻击
        if len(X_batch_adv) > 0:
           X_batch_adv = self.pgd_attack(self.model, X_batch_adv, y_batch_adv,self.pgd_epsilon, self.pgd_alpha, self.pgd_iters)
           X_batch[adv_mask] = X_batch_adv
    ...
    def pgd_attack(self,model, X, y, epsilon, alpha, num_iters):
        """
    Performs the PGD attack on the given model.
    Inputs:
     - model: The model to be attacked.
     - X: Input data, of shape (N, C, H, W).
    - y: True labels for X, of shape (N,).
    - epsilon: Maximum perturbation allowed for each pixel.
    - alpha: Step size for each iteration of the attack.
    - num_iters: Number of iterations for the attack.
    Returns:
    - X_adv: Adversarial examples, of shape (N, C, H, W).
        """
